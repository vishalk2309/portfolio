...
import { useJobSubscribe, POPUP_KEY } from "../hooks/useJobSubscribe";
...
// Small corner popup nudging visitors to subscribe to job updates.
// Shows once per browser: dismissing or subscribing keeps it hidden.
export default function JobSubscribePopup({ delay = 8000 }) {
  const [open, setOpen] = useState(false);
  const { email, setEmail, status, msg, subscribe } = useJobSubscribe();
    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          role="dialog"
          aria-label="Subscribe to job updates"
        >
              ...
              <>
                <div className="flex items-center gap-2 pr-6">
                  ...
                  <p className="text-sm font-bold text-white">
                    New jobs, straight to you
                  </p>
                </div>
                <p className="mt-1 text-xs leading-relaxed text-white/55">
                  Get an email whenever a new opening is posted. No spam.
                </p>
