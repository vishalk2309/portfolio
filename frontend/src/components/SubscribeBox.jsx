...
/** Always-visible subscribe form for the blog pages (email → subscribe-blog). */
export default function SubscribeBox() {
  ...
  const subscribe = async (e) => {
    ...
    try {
      ...
      setStatus("done");
      // Also stop the popup from nagging once subscribed here.
      try {
        localStorage.setItem("blog-sub-dismissed", "1");
      } catch {
        /* ignore */
      }
    } catch (err) {
    ...
    <div className="glass rounded-3xl p-6 sm:p-8">
      {status === "done" ? (
        <p className="text-center font-semibold text-emerald-400">
          ✓ You&rsquo;re subscribed! We&rsquo;ll email you when a new post goes
          live.
        </p>
      ) : (
        <>
          <h3 className="text-xl font-bold text-white">Subscribe to the blog</h3>
          <p className="mt-1 text-sm text-white/60">
            Get an email whenever a new post is published — no spam.
          </p>
